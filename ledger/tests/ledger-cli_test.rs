...
fn list_test_files(dir: &str) -> std::io::Result<Vec<String>> {
    ...
    list_test_files_recursive(dir, &mut files)?;
    Ok(files)
}

fn list_test_files_recursive(dir: &str, files: &mut Vec<String>) -> std::io::Result<()> {
    for entry in fs::read_dir(dir)? {
        ...
        if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("test") {
            ...
            list_test_files_recursive(path.to_str().unwrap(), files)?;
        }
    }
    Ok(())
}
